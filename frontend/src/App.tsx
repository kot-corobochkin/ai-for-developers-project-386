...
function BookingsAdmin() {
  ...
  return <section><div className="toolbar"><div><p className="eyebrow">Расписание</p><h2>Ближайшие бронирования</h2></div><select className="select" value={status} onChange={(e) => setStatus(e.target.value as BookingStatus | "")}><option value="">Все статусы</option><option value="confirmed">Подтверждённые</option><option value="cancelled">Отменённые</option></select></div>{query.isLoading && <Loading />}{query.error && <ErrorBox error={query.error} />}<div className="admin-table">{query.data?.map((booking) => <div className="table-row booking-row" key={booking.id}><div className="row-main"><div className="date-tile"><strong>{format(parseISO(booking.startsAt), "d")}</strong><small>{format(parseISO(booking.startsAt), "MMM")}</small></div><div><strong>{booking.eventTypeTitle}</strong><small>{booking.guestName} · {booking.guestEmail}</small></div></div><span>{formatDateTime(booking.startsAt)} – {formatDateTime(booking.endsAt).split(", ").pop()}</span><Badge tone={booking.status === "confirmed" ? "success" : "neutral"}>{booking.status === "confirmed" ? "Подтверждено" : "Отменено"}</Badge></div>)}</div>{!query.data?.length && !query.isLoading && <EmptyState>Ближайших бронирований нет.</EmptyState>}</section>;
}
